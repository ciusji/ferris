...
// by crating a Box<T>. A box is a smart pointer to a heap allocated value of type `T`. When a box
...
pub fn box_stack_heap() {
    ...
    println!("Point occupied {} bytes on the stack", mem::size_of_val(&point));
    println!("Rectangle occupied {} bytes on the stack", mem::size_of_val(&rectangle));
